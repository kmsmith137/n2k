...
extern ksgpu::Array<std::complex<int>> make_random_unpacked_e_array(int T, int F, int S);       // returns shape (T,F,S,2)
extern ksgpu::Array<std::complex<int>> unpack_e_array(const ksgpu::Array<uint8_t> &E_in, bool offset_encoded);     // shape-preserving
extern ksgpu::Array<uint8_t> pack_e_array(const ksgpu::Array<std::complex<int>> &E_in, bool offset_encoded);       // shape-preserving
